...
def chatbot_config():
    ...
    return jsonify({
        ...
        "finaleMsg": CHARACTER.get("finale_message", "Come visit France next time!"),
...
def question_retry_response(stage, original, ambiguity_options=None):
    ...
    default_examples = {
        Stage.STUDENT_QUESTION_1.value: "Do you like sandwiches?",
        Stage.STUDENT_QUESTION_2.value: "Do you like macarons?",
        Stage.STUDENT_QUESTION_3.value: "Do you like baguettes?",
    }
    ...
    if attempt >= QUESTION_CONFIRMATION_ATTEMPT:
        ...
        if stage == Stage.STUDENT_QUESTION_3.value:
            candidate_questions.extend(
                f"Do you like {option}?" for option in (ambiguity_options or [])
            )

        configured_example = str(retry_examples.get(stage, "")).strip()
        if configured_example:
            candidate_questions.append(configured_example)
        candidate_questions.append(default_examples.get(stage, "Do you like baguettes?"))

        fallback_foods = {
            Stage.STUDENT_QUESTION_1.value: ["sandwiches", "macarons", "baguettes"],
            Stage.STUDENT_QUESTION_2.value: ["macarons", "sandwiches", "baguettes"],
            Stage.STUDENT_QUESTION_3.value: ["baguettes", "sandwiches", "macarons"],
        }
        candidate_questions.extend(
            f"Do you like {food_name}?"
            for food_name in fallback_foods.get(stage, ["baguettes"])
        )

        asked_foods = set(session.get("asked_foods", []))
        suggested_question = "Do you like baguettes?"
        for candidate_question in candidate_questions:
            food = find_food(candidate_question)
            if food and food["key"] not in asked_foods:
                suggested_question = f"Do you like {food['display_name']}?"
                break
...
def chat():
    ...
    if stage in question_stages:
        food_resolution = resolve_known_food(message, alternatives)
        ...
        if not food:
            food = classify_open_food_candidates(message, alternatives)
            if food:
